...
# --- CONTEXT PROCESSOR (DEBE IR AQUÍ, DESPUÉS DE DEFINIR 'app') ---
...
def inject_global_vars():
    ...
    return dict(VERSION=VERSION_APP, SUPERVISORES_CONTACTO=SUPERVISORES_CONTACTO)
# ----------------------------------------------------------------------
...
def seed_data():
    ...
    db.session.add(GlobalSettings(current_escoba_name="Ninguno", current_escoba2_name=None))
    db.session.commit()
...
def force_status_change(driver_id):
    ...
    if driver and new_status: driver.current_status = new_status; db.session.commit(); flash(f'Estado cambiado a {new_status}', 'info')
    return redirect(url_for('dashboard'))
...
def next_turn():
    ...
    if not candidatos: flash('Nadie disponible hoy.', 'danger')
        ...
        elegido.last_escoba_date = datetime.now(); db.session.commit(); flash(f'Nuevo Turno: {elegido.name}', 'success')
    return redirect(url_for('dashboard'))
